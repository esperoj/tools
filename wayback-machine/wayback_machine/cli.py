...
class Reporter:
    ...
    def __enter__(self):
        if self.mode == "curses" and sys.stdout.isatty() and curses:
            ...
            self._write(
                0,
                f"=== Wayback Machine: SPN2 ({self.workers} Workers) ===",
                curses.A_REVERSE,
            )
        return self
        ...
        with self.lock:
            if self.mode == "json":
                ...
                try:
                    ...
                    icon = (
                        "▶"
                        if status in ("INIT", "WAIT", "SAVE", "POLL")
                        else "✅"
                        if status == "OK"
                        else "❌"
                    )
                    self._write(
                        y, f"{icon} W{slot + 1} | {url}", curses.A_BOLD
                    )
...
def run_save(args: argparse.Namespace):
    ...
    with reporter:
        results = client.save_batch(
            jobs, default_opts, on_event=reporter.handle
        )
        reporter.hold()
...
def main():
    ...
    global_parser.add_argument(
        "--workers", type=int, default=4, help="Concurrent workers"
    )
    global_parser.add_argument(
        "--ui",
        choices=["basic", "json", "curses"],
        default="basic",
        help="Output format",
    )
    ...
    save_parser = subparsers.add_parser(
        "save", parents=[global_parser], help="Archive URLs via SPN2 API"
    )
    save_parser.add_argument(
        "urls", nargs="*", help="URLs to archive (or via stdin)"
    )
