...
class WaybackMachine:
    ...
    def __init__(
        ...
        proxy_prefix: str = "https://proxy.esperoj.workers.dev/",
        max_workers: int = 4,
        dry_run: bool = False,
    ):
        ...
        self.api_secret = api_secret or os.getenv("INTERNET_ARCHIVE_SECRET_KEY")
    ...
    def _get_spn2_headers(self) -> dict[str, str]:
        ...
        if not self.dry_run and not (self.api_key and self.api_secret):
            raise ValueError("Save/SPN2 features require INTERNET_ARCHIVE_ACCESS_KEY and SECRET_KEY.")
        return {
            "Accept": "application/json",
            "Authorization": f"LOW {self.api_key}:{self.api_secret}",
        }

    def _curl(self, executor: CurlExecutor, url: str, method: str = "GET", headers: dict = None, data: Any = None) -> dict:
    ...
    def save_batch(
        ...
        def _worker(job_input: dict | str) -> dict:
            job = {"url": job_input} if isinstance(job_input, str) else job_input
            url = job["url"]
            opts = {**defaults, **{k: v for k, v in job.items() if k != "url"}}
            
            with lock: slot = slots.pop(0)
            res_data = {"url": url, "archive_url": None, "error": None}

            try:
                if on_event: on_event(url, slot, "INIT", "Starting save job")
                url_res = self._save_single(url, opts, slot, on_event)
                res_data["archive_url"] = url_res
                if on_event: on_event(url, slot, "OK", url_res)
            except Exception as e:
                res_data["error"] = str(e)
                if on_event: on_event(url, slot, "FAIL", str(e))
            finally:
                with lock: slots.append(slot)
    ...
    def _save_single(self, url: str, opts: dict, slot: int, on_event: Callable | None) -> str:
        """Internal multistep SPN2 pipeline for a single URL."""
        if self.dry_run:
            if on_event: on_event(url, slot, "SAVE", "Simulating API request...")
            time.sleep(1.5)
        ...
        while True:
            if time.time() - start_time > 900: raise RuntimeError("Timeout waiting for SPN2 queue.")
            if on_event: on_event(url, slot, "WAIT", "Checking account availability...")
            
            status_data = self._curl(executor, f"https://web.archive.org/save/status/user?_t={int(time.time())}", headers=headers)
            if status_data.get("available", 0) >= 2: break
            time.sleep(sleep_dynamic)

        # 2. SAVE: Submit URL to be archived
        if on_event: on_event(url, slot, "SAVE", "Submitting to archive queue...")
        save_res = self._curl(executor, "https://web.archive.org/save", method="POST", headers=headers, data={"url": url, **opts})
        job_id = save_res.get("job_id")
        if not job_id: raise RuntimeError(f"Missing job_id in SPN2 response: {save_res}")

        # 3. POLL: Wait for completion
        start_time = time.time()
        while True:
            if time.time() - start_time > 960: raise RuntimeError("Timeout polling job status.")
            if on_event: on_event(url, slot, "POLL", f"Polling job {job_id} [{int(time.time() - start_time)}s]")
            
            poll_data = self._curl(executor, f"https://web.archive.org/save/status/{job_id}", headers=headers)
    ...
    def check_index(self, url: str) -> dict:
        ...
        raise NotImplementedError("Index checking will be implemented in a future release.")

    def search_cdx(self, query: str) -> list[dict]:
        """Placeholder for the CDX Server API."""
        raise NotImplementedError("CDX querying will be implemented in a future release.")
